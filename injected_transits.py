...
detrending = 'lowess_partial' # Can be 'poly', 'lowess_full', 'lowess_partial', 'TESSflatten' OR 'None'
...
single_target_ID = ["HD 24636", "HIP 1113"]
...
for target_ID in all_target_IDs:
    ...
    #        flux_cut = combined_flux
        ...
        if detrending == 'lowess_partial':
            ...
            lowess_flux_section = lowess[:,1]
            plt.plot(t_section, lowess_flux_section, '-')
            overplotted_detrending_fig.savefig(save_path + "{} - Overplotted lowess detrending - partial lc - boxsize 30".format(target_ID))
    ...
    #        lc = np.vstack((lc_30min.time, combined_flux, lc_30min.flux_err)).T
            ...
            TESSflatten_fig.savefig(save_path + '{} - injected {}R transit TESSflatten lightcurve - 2day Per.png'.format(target_ID, params.rp))
    ...
    #    periodogram_fig, ax = plt.subplots(1, 1, figsize=(8, 4))
        ...
        if flatten == True:
            ...
            periodogram_fig.savefig(save_path + '{} - BLS Periodogram - {}R - 8 day Per.png'.format(target_ID, params.rp))
        plt.close(periodogram_fig)
    #    plt.show()    
    ...
    ##    ################################## Phase folding ##########################
        ...
        phase_fold_plot(t_cut, BLS_flux, rot_period.value, rot_t0.value, target_ID, save_path, '{} with injected 8 day transit folded by rotation period - {}R ratio'.format(target_ID, params.rp))
        phase_fold_plot(t_cut, BLS_flux, period.value, t0.value, target_ID, save_path, '{} with injected 8 day transit folded by Periodogram Max - {}R ratio'.format(target_ID, params.rp))
    except TypeError:

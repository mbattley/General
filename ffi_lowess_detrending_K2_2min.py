...
#                lc_30min = lc_30min[~nancut]
                ...
                try:
                    if pipeline == 'DIA':
                        ...
                        sap_lc, pdcsap_lc, sector = two_min_lc_download(target_ID, sector = sector, from_file = True)
                        lc_30min = pdcsap_lc
                        nancut = np.isnan(lc_30min.flux) | np.isnan(lc_30min.time)
                        lc_30min = lc_30min[~nancut]
...
save_path = '/Users/mbattley/Documents/PhD/New detrending methods/Smoothing/lowess/K2_2min_xmatch/' # On laptop
sector = 13
pipeline = '2min'

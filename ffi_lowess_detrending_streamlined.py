...
from utility_belt import find_freqs
...
#                lc_30min = lc_30min[~nancut]
    ...
    #                    lc_30min = lcf.PDCSAP_FLUX
                        ...
                        target_ID_pad = str(target_ID).rjust(16,'0')
                        filename = 'hlsp_qlp_tess_ffi_s00{}-{}{}{}{}_tess_v01_llc.fits'.format(sector,target_ID_pad[0:4],target_ID_pad[4:8],target_ID_pad[8:12],target_ID_pad[12:16])
                        lc_30min = get_lc_from_fits(filename, source = pipeline,clean=True)
                    else: 
...
#            if multi_sector == False and pipeline != '2min':
...
#            target_ID = 'HD 42270'
            ...
            # From file
            if extra_data != False:
                roseta_target_list = extra_data['ID']
                i = list(roseta_target_list).index(target_ID)
#                i = find(roseta_target_list==target_ID)
                p_rot = extra_data['LS_period'][i]
            
            # From Lomb-Scargle
            if find_freq == True:
                freq_list = find_freqs(lc_30min.time,lc_30min.flux,plot_ls_fig = False,target_ID=target_ID)
                p_rot = 1/freq_list[0]
...
#                params.t0 = random.uniform(-14,14)#-10.0                      #time of inferior conjunction
##                params.t0 = -13.085
#                params.per = 8.0
#                params.rp = 0.1
...
#            plt.show()
            ...
            ################## Saving lcs and period data to file  ###################
            ...
            with open(save_path+'Period_info_table_new.csv','a') as f:
        ...
        print('Number analysed = {}'.format(num_done))
        num_done += 1
    if find_freq == True:
        np.savetxt('young_star_freqs_S{}_test.txt'.format(sector),freq_table)
...
save_path = '/Users/mbattley/Documents/PhD/Roseta/' # On laptop
sector = 14
pipeline = 'QLP'
...
with open(save_path+'Period_info_table_new.csv','w') as f:
...
filename = 'rotator_tic_s14_only.csv'
data = Table.read(filename, format='ascii.csv')
#roseta_target_list = list(data['ID'][0:2])
roseta_target_list = [11695746,188518505,138717209,59378294]

print(roseta_target_list)

start = time.time()
t_cut_f, BLS_flux_f, phase_f, epoch_f, period_f = ffi_lowess_detrend(save_path = save_path, sector = sector, target_ID_list = roseta_target_list, pipeline = pipeline, multi_sector = multi_sector, use_peak_cut = use_peak_cut, binned = binned, transit_mask = transit_mask, injected_planet = injected_planet, detrending = detrending, n_bins = n_bins, extra_data = data)
end = time.time()

...
binned = False
transit_mask = False
injected_planet = 'exo_archive'      # Can be 'exo_archive', 'set_period', 'set_depth', 'user_defined' or False
detrending = 'lowess_partial' # Can be 'poly', 'lowess_full', 'lowess_partial', 'TESSflatten', 'wotan' OR 'None'
single_target_ID = ['AO Men']
######################################################################################
...
for target_ID in single_target_ID:
    try:
        try:
            lc_30min, filename = diff_image_lc_download(target_ID, sector, plot_lc = True, save_path = save_path, from_file = False)
            pipeline = 'Diff_Image'
...
#        model = BLS(lc_30min.time*u.day, BLS_flux)
    ...
    #        stellar_type = 'F or G'
            ...
            if injected_planet == 'user_defined':
                ...
                params.rp = 0.1                      #planet radius (in units of stellar radii) - Try between 0.01 and 0.1 (F/G) or 0.025 to 0.18 (K/M)
...
#        combined_flux = np.array(lc_30min.flux)/np.median(lc_30min.flux)
        if use_peak_cut == True:
            peaks, peak_info = find_peaks(combined_flux, prominence = 0.001, width = 120)
            #peaks = np.array([64, 381, 649, 964, 1273])
            troughs, trough_info = find_peaks(-combined_flux, prominence = -0.001, width = 120)
...
#        period = 14.05
        ...
        eye_balling_fig.savefig(save_path + '{} - Full eyeballing fig - {}R - {}d Per.png'.format(target_ID, params.rp, params.per))

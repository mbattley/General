...
#                    lc_30min = raw_FFI_lc_download(target_ID, sector, plot_tpf = False, plot_lc = True, save_path = save_path, from_file = False)
                if pipeline == 'CDIPS':
                    lc_30min, target_ID, sector = get_lc_from_fits(filename=input_filename, source = pipeline, save_path = save_path)
                    nancut = np.isnan(lc_30min.flux) | np.isnan(lc_30min.time)
                    lc_30min = lc_30min[~nancut]
                    print(target_ID)
#                elif pipeline == 'QLP':
...
#            print('Generated new cut-down peak powers')
...
#            plt.scatter(binned_phase, binned_lc, c='r', s=4)
        ...
        eye_balling_fig.tight_layout() #This step seems to take ages on ngtshead
...
#            sensitivity_table.add_row([target_ID,sector,pipeline,params.per,params.a,params.rp,period,np.max(results.power),period_2.value,period_3.value])
        with open(save_path+'Period_info_table2.csv','a') as f:
            data_row = [target_ID,sector,np.max(results.power),period,transit_info['transit_times'][0],period_2.value,period_3.value, p_rot]
...
save_path = '/home/u1866052/Lowess detrending/CDIPS/cam1/' # ngtshead
#save_path = '/Users/mbattley/Documents/PhD/New detrending methods/Smoothing/lowess/CDIPS/' # On laptop
sector = 6
pipeline_used = 'CDIPS'
...
with open(save_path+'Period_info_table2.csv','w') as f:
...
with open(save_path+'Failed_lcs2.csv','w') as f:
    info_row = ['Filename', 'Sector']
    writer = csv.writer(f, delimiter=',')
    writer.writerow(info_row)
...
for sector in sector_list:
    for ccd in ccd_list:
        directory_str = '/tess/photometry/cdips/s000{}/cam{}_ccd{}'.format(sector,camera,ccd)
        #directory_str = '/Users/mbattley/Documents/PhD/Python/Test_folder/s0006/cam{}_ccd{}'.format(camera,ccd)
        directory = os.fsencode(directory_str)
        
        
        #filenames_table = Table.read(save_path+'Failed_lcs_cam1.csv',format = 'ascii.csv')
        #failed_lcs = filenames_table['Filename']
        
        for file in os.listdir(directory):
             filename = os.fsdecode(file)
             if filename.endswith(".fits") and filename[58] == str(camera): 
                 hdul = fits.open(directory_str+'/'+filename)
                 tic = hdul[0].header['TICID']
                 save_path = '/home/u1866052/Lowess detrending/CDIPS/cam{}/'.format(camera)
                 if os.path.exists(save_path + '{} - Full eyeballing fig.pdf'.format(tic)) == False:
                     try:
                         t_cut_f, BLS_flux_f, phase_f, epoch_f, period_f = ffi_lowess_detrend(save_path = save_path, sector = sector, target_ID_list = [], pipeline = pipeline_used, multi_sector = multi_sector, use_TESSflatten = use_TESSflatten, use_peak_cut = use_peak_cut, binned = binned, transit_mask = transit_mask, injected_planet = injected_planet, injected_rp = rp, injected_per = 8.0, detrending = detrending, n_bins = n_bins, input_filename = directory_str+'/'+filename)
                     except:
                         with open(save_path+'Failed_lcs2.csv','a') as f:
                             data_row = [filename, filename[53]]
                             writer = csv.writer(f, delimiter=',')
                             writer.writerow(data_row)
                 num_lc_analysed += 1
                 print('Number of lightcurves analysed = {}'.format(num_lc_analysed))
                 current_time = time.time()
                 print('Current elapsed time = {}s'.format(current_time - start))
                 continue
             else:
                 continue
end = time.time()
